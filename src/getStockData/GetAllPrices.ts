...
async function fetchPrice(ticker: string) {
  ...
  currentDate.setFullYear(currentDate.getFullYear() - 999);
  ...
  return response && response.data && response.data[0]
    ? response.data[0].prices
    : [];
}

function formatPrices(prices: any) {
  const formatDate = "DD/MM/YY HH:mm";
  const formatDatePrices = prices.map((price: any) => ({
    ...price,
    timestamp: moment(price.date, formatDate).unix(),
    date: moment(price.date, "DD/MM/YY HH:mm").format("DD/MM/YYYY"),
  }));
  const sortPricesByDate = formatDatePrices.sort(
    (a: any, b: any) => a.timestamp - b.timestamp
  );
  return sortPricesByDate;
}

export default async function getAllPrices(ticker: any) {
  const dataPrice = await fetchPrice(ticker);
  const formattedPrices = formatPrices(dataPrice);
