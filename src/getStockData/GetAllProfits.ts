...
async function fetchProfitsHistory(ticker: any) {
  const lastYear = moment().subtract(1, "year").format("YYYY");
  const url = `https://statusinvest.com.br/acao/getdre?code=${ticker}&type=0&futureData=false&range.min=1500&range.max=${lastYear}`;
  const method = "GET";
  const headers = {
...
function formatProfitsHistory(profitsHistory: any) {
  const dataProfits = profitsHistory.find(
...
export default async function getAllProfits(ticker: any) {
  const dataProfitsHistory = await fetchProfitsHistory(ticker);
  const formattedProfitsHistory = formatProfitsHistory(dataProfitsHistory);
  return formattedProfitsHistory;
}
